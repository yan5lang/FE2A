...
public class FileSwapHelper {
    ...
    private boolean needNewFile(String newFile){
        if(currentFileName==null || newFile==null ||  newFile.length()<14)
            return false;
        String hour1 = StringUtils.substring(currentFileName,11,13);
        String hour2 = StringUtils.substring(newFile,11,13);
        return !hour1.equals(hour2);
    }
    ...
    private String getSaveFilePath(String fileName) {
        ...
        fullPath.append(FileUtils.getMainDirName());
        fullPath.append(BASE_VIDEO);
//        fullPath.append(com.szetn.util.FileUtils.getInst().getPhotoSavedPath());
...
//            } while (checkCardSpace(checkPath));
    ...
    private boolean checkCardSpace(String filePath) {
        ...
        if (freeSpace < totalSpace * 0.03) {
//            Log.e("angcyo-->", filePath + " 剩余空间不足3%，开始清理空间...");
